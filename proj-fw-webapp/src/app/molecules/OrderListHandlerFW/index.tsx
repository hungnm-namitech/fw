...
import { DivValue, ItemGroup } from '@/app/types/entities';
...
export default function OrderListHandlerFW({ session }: OrderListHandlerProps) {
  const {
    register,
    formState: { errors },
    control,
  ...
  const { data: itemGroups, error } = useSWR(
    'item-group-names',
    () => ItemGroupApi.allName(session || ''),
    {
      suspense: true,
    },
  );
  const { data: divValues } = useSWR(
    'all-div-values',
    useCallback(() => DivValues.all(''), []),
  );
  ...
  const searchFields: SearchFieldProps[] = [
    {
      ...
      control: control,
      options:
        divValues?.map((item: DivValue) => {
          return {
            label: item?.divValueName?.toString() || '',
            value: item?.divValue?.toString() || '',
          };
        }) || [],
      className: '!mt-1',
    },
    {
      ...
      name: 'customerIds',
      error: errors?.customerIds?.message?.toString(),
      control: control,
      options: [
        {
          label: '全て',
    ...
    },
    {
      ...
      classfield: 'w-44 mx-2 mt-1.5',
      placeholder: 'アイテム名',
      ...register('itemGroupIds'),
      name: 'itemGroupIds',
      error: errors?.itemGroupIds?.message?.toString(),
      control: control,
      isMulti: true,
  ...
  const { data: data, isLoading } = useSWRImmutable(
    ...
    () =>
      Orders.getList(session || '', {
        ...watchAllFields,
        statuses: findIndices(
          watchAllFields.statuses || [],
          (value: boolean) => value,
        ).join(','),
        itemGroupIds: (
          watchAllFields?.itemGroupIds?.map((item: any) => {
            return item.value;
          }) || []
        ).join(','),
