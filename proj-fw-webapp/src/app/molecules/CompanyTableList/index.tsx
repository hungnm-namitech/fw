            ...
            <tr key={row?.companyCd}>
              {columns.map(head => (
                <td
                  ...
                  key={
                    row[head.key as keyof typeof row] + head.key + row?.companyCd
                  }
                >
                <div className={clsx('text-[#222222] text-sm font-normal leading-5 overflow-ellipsis',
                  head.className
                )}>
                  <p id={"data_" + head.key + row?.companyCd}>{row[head.key as keyof typeof row]}
                  </p>
                    <Tooltip
                      anchorSelect={"#data_" + head.key + row?.companyCd}
                      content={
                        head.key === 'companyName'
                        ? row.companyName
                        : head.key === 'companyNameKana'
                        ? row.companyNameKana
                        : head.key === 'address'
                        ? row.address
                        : ''
